...
export const Charts = ({ params, isActive}) => {
  ...
  const getCandle = async ({ type, from, to }) => {
    ...
    if (inUSD) {
      if (type === "daily") {
        rateUSD = await axios
          .get(`https://min-api.cryptocompare.com/data/v2/histoday?fsym=RECH&tsym=USD&toTs=${moment().unix()}&limit=2000`)
          .then((RateDate) => {
            const dateObject = {};
        ...
        rateUSD = await axios
          .get(`https://min-api.cryptocompare.com/data/v2/histohour?fsym=RECH&tsym=USD&toTs=${moment().unix()}&limit=2000`)
          .then((RateDate) => {
            const dateObject = {};
            RateDate.data.Data.Data.forEach((e) => {
