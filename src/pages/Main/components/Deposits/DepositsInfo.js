...
export const DepositsInfo = ({ isActive,
  ...
  const odexUrl = `https://odex.ooo/trade/${(symbol3 === 'OUSD' ? 'RECH' : symbol3)}/OUSD`;
  let oswapUrl = 'https://oswap.io/#/swap';

  switch (symbol3) {
    case 'OUSD':
      oswapUrl += '/I7UKETQTWW5H25BPLIIXLZTQBAKYTTN2?reverse=1'; // OUSD to RECH
      break;
    case 'OBIT':
    ...
    case 'OAU':
      ...
      break
    default:
      oswapUrl += `/${encodeURIComponent(asset)}`; // RECH to any (doesn't support reverse)
      break;
  }
