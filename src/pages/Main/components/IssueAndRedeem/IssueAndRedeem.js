          ...
          <Input.Group compact size="large">
            ...
            <Form.Item style={{ width: "50%", marginBottom: 0 }}>
              <Select size="large" placeholder="Select token" value={fromAsset} onChange={(v) => setFromAsset(v)} style={{ width: "100%" }} >
                {reserve ? (
                  <>
                    <Select.Option value={reserve_asset}> {reserve_asset === "base" ? <RECHIcon width="1em" height="1em" style={{ marginRight: 3, marginBottom: -1.5 }} /> : null} {reserve_asset_symbol}</Select.Option>
            ...
            <Form.Item style={{ width: "50%", marginBottom: 0 }}>
              <Select size="large" placeholder="Select token" value={toAsset} onChange={(v) => setToAsset(v)} style={{ width: "100%" }}>
                {currentPairs?.map((p, i) => {
                  return <Select.Option key={"option" + i} value={p.asset}>{p.type > 0 ? <CoinIcon width="1em" height="1em" style={{ marginRight: 5, marginBottom: -1.5 }} pegged={peggedCurrency} type={p.type} /> : (p.asset === "base" ? <RECHIcon width="1em" height="1em" style={{ marginRight: 3, marginBottom: -1.5 }} /> : null)} {p.symbol || p.asset}</Select.Option>
                })}
              </Select>
            </Form.Item>
