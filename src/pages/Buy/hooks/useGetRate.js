      ...
      if (activeCurrency !== undefined && inited) {
        ...
        if (activeCurrency !== "RECH") {
          if (ranges && ranges.min) {
            ...
            const rateData = await axios.get(
              `https://api.simpleswap.io/v1/get_estimated?api_key=${config.SIMPLESWAP_API_KEY}&currency_from=${activeCurrency}&currency_to=RECH&amount=${min}`
            ).catch(() => {
            ...
            const rateData = await axios.get(
              `https://api.simpleswap.io/v1/get_estimated?api_key=${config.SIMPLESWAP_API_KEY}&currency_from=${activeCurrency}&currency_to=RECH&amount=1`
            ).catch(() => {
              console.log("get_estimated error")
            });
