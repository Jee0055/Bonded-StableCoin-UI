...
import { RECHIcon } from "components/RECHIcon/RECHIcon";
...
export const IssueAndRedeemAllTokens = () => {
  ...
  useEffect(() => {
    ...
    if (assetsType[toAsset] && assetsType[toAsset].type === 2) {
      ...
      description = t("trade.tabs.buy_redeem.desc_RECH", "The main token of rechain_inc network.")
    }
          ...
          <Input.Group compact size="large">
            ...
            <Form.Item style={{ width: "60%", marginBottom: 0 }}>
              <Select optionFilterProp="children" showSearch={true} size="large" placeholder={t("trade.tabs.buy_redeem.select_token", "Select token")} value={fromAsset} onChange={(v) => setFromAsset(v)} style={{ width: "100%" }} dropdownStyle={{ fontWeight: 400 }} className="select-issue">
                {fromList.map((asset) => <Select.Option key={"send-" + asset} value={asset}> {asset in assetsType ? <CoinIcon width="1em" height="1em" style={{ marginRight: 5, marginBottom: -1.5 }} pegged={assetsType[asset].peggedToCurrency} type={assetsType[asset].type} /> : (asset === "base" ? <RECHIcon width="1em" height="1em" style={{ marginRight: 3, marginBottom: -1.5 }} /> : null)} {symbolList[asset]?.symbol || asset}</Select.Option>)}
              </Select>
            </Form.Item>
          ...
          <Input.Group compact size="large" style={{ borderColor: "#0137FF" }}>
            ...
            <Form.Item style={{ width: "60%", marginBottom: 0 }}>
              <Select optionFilterProp="children" showSearch={true} size="large" bordered={true} placeholder={t("trade.tabs.buy_redeem.select_token", "Select token")} value={toAsset} onChange={(v) => setToAsset(v)} style={{ width: "100%", borderColor: "#0137FF" }} dropdownStyle={{}} className="select-issue">
                {currentPairs?.map((p, i) => <Select.Option key={"option" + i} value={p.asset}> {assetsType[p.asset] ? <CoinIcon width="1em" height="1em" style={{ marginRight: 5, marginBottom: -1.5 }} pegged={assetsType[p.asset].peggedToCurrency} type={assetsType[p.asset]?.type} /> : (p.asset === "base" ? <RECHIcon width="1em" height="1em" style={{ marginRight: 3, marginBottom: -1.5 }} /> : null)} {symbolList[p.asset]?.symbol || p.asset} {assetsType[p.asset]?.info || null} </Select.Option>)}
              </Select>
            </Form.Item>
          ...
          {!activeWallet && <div style={{ textAlign: "center", marginTop: 10 }}>
            <Text type="secondary" style={{ fontSize: 14, display: "block" }}>
              <Trans i18nKey="trade.tabs.buy_redeem.open_wallet">
                Clicking "Exchange" will open your rechain_inc wallet. <a
