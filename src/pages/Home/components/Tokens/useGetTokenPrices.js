...
export const useGetTokenPrices = (list, data, balances) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const RECH_USD = isBot ? await axios.get(config.BUFFER_URL + "/get_data_feed/" + config.RATE_ORACLE + "/RECH_USD").then(response => response.data.data)
      ...
      for (const pegged in info) {
        if (info[pegged].reserve === "RECH") {
          newFund[pegged] = info[pegged].pFundInReserve * RECH_USD;
          newInterest[pegged] = info[pegged].p2InReserve * RECH_USD;
          newStable[pegged] = info[pegged].p3InReserve * RECH_USD;
