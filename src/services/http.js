...
class httpHub {
	constructor() {
		this.hubUrl = `https://${config.TESTNET ? "testnet." : ""}rechain_inc.org/api`;
	...
	async getAssetBySymbol(tokenRegistryAddress, symbol) {
		...
		if (symbol === 'RECH' || symbol === 'MBYTE' || symbol === 'KBYTE' || symbol === 'BYTE') {
			return 'base';
		}
	...
	async getDecimalsBySymbolOrAsset(tokenRegistryAddress, symbolOrAsset) {
		...
		if (symbolOrAsset === 'base' || symbolOrAsset === 'RECH') {
			return 9;
		}
