...
export const ChangeParamsModal = ({
  ...
  const validateParams = {
    ...
    decision_engine_aa: {
      validator: (value) => {
        return rechain_inc.utils.isValidAddress(value);
      },
    },
    sf_capacity_share: {
      validator: (value) => value >= 0 && value <= 100,
  ...
  useEffect(() => {
    if (param === "oracles" && oracles) {
      ...
      if (oracle1 && feed_name1 && op1 && rechain_inc.utils.isValidAddress(oracle1)) {
        oraclesArray.push(oracle1 + op1 + feed_name1);
      }
      if (oracle2 && feed_name2 && op2 && rechain_inc.utils.isValidAddress(oracle2)) {
        oraclesArray.push(oracle2 + op2 + feed_name2);
      }
      if (oracle3 && feed_name3 && op3 && rechain_inc.utils.isValidAddress(oracle3)) {
